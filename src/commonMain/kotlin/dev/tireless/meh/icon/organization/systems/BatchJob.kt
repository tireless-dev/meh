package dev.tireless.meh.icon.organization.systems

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val BatchJob: ImageVector
    get() {
        val current = _batchJob
        if (current != null) return current

        return ImageVector.Builder(
            name = "MehTheme.BatchJob",
            defaultWidth = 32.0.dp,
            defaultHeight = 32.0.dp,
            viewportWidth = 32.0f,
            viewportHeight = 32.0f,
        ).apply {
            // M32 26 v-2 h-2.1 a5 5 0 0 0 -.73 -1.75 l1.49 -1.5 -1.42 -1.4 -1.49 1.48 A5 5 0 0 0 26 20.1 V18 h-2 v2.1 a5 5 0 0 0 -1.75 .73 l-1.5 -1.49 -1.4 1.42 1.48 1.49 A5 5 0 0 0 20.1 24 H18 v2 h2.1 a5 5 0 0 0 .73 1.75 l-1.49 1.5 1.42 1.4 1.49 -1.48 A5 5 0 0 0 24 29.9 V32 h2 v-2.1 a5 5 0 0 0 1.75 -.73 l1.5 1.49 1.4 -1.42 -1.48 -1.49 A5 5 0 0 0 29.9 26z m-7 2 a3 3 0 1 1 0 -6 3 3 0 0 1 0 6 m-5 -11 h-8 a2 2 0 0 1 -2 -2 V4 a2 2 0 0 1 2 -2 h8 a2 2 0 0 1 2 2 v11 a2 2 0 0 1 -2 2 m-8 -2 h8 V4 h-8z
            path(
                fill = SolidColor(Color(0xFF000000)),
            ) {
                // M 32 26
                moveTo(x = 32.0f, y = 26.0f)
                // v -2
                verticalLineToRelative(dy = -2.0f)
                // h -2.1
                horizontalLineToRelative(dx = -2.1f)
                // a 5 5 0 0 0 -0.73 -1.75
                arcToRelative(
                    a = 5.0f,
                    b = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    dx1 = -0.73f,
                    dy1 = -1.75f,
                )
                // l 1.49 -1.5
                lineToRelative(dx = 1.49f, dy = -1.5f)
                // l -1.42 -1.4
                lineToRelative(dx = -1.42f, dy = -1.4f)
                // l -1.49 1.48
                lineToRelative(dx = -1.49f, dy = 1.48f)
                // A 5 5 0 0 0 26 20.1
                arcTo(
                    horizontalEllipseRadius = 5.0f,
                    verticalEllipseRadius = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    x1 = 26.0f,
                    y1 = 20.1f,
                )
                // V 18
                verticalLineTo(y = 18.0f)
                // h -2
                horizontalLineToRelative(dx = -2.0f)
                // v 2.1
                verticalLineToRelative(dy = 2.1f)
                // a 5 5 0 0 0 -1.75 0.73
                arcToRelative(
                    a = 5.0f,
                    b = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    dx1 = -1.75f,
                    dy1 = 0.73f,
                )
                // l -1.5 -1.49
                lineToRelative(dx = -1.5f, dy = -1.49f)
                // l -1.4 1.42
                lineToRelative(dx = -1.4f, dy = 1.42f)
                // l 1.48 1.49
                lineToRelative(dx = 1.48f, dy = 1.49f)
                // A 5 5 0 0 0 20.1 24
                arcTo(
                    horizontalEllipseRadius = 5.0f,
                    verticalEllipseRadius = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    x1 = 20.1f,
                    y1 = 24.0f,
                )
                // H 18
                horizontalLineTo(x = 18.0f)
                // v 2
                verticalLineToRelative(dy = 2.0f)
                // h 2.1
                horizontalLineToRelative(dx = 2.1f)
                // a 5 5 0 0 0 0.73 1.75
                arcToRelative(
                    a = 5.0f,
                    b = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    dx1 = 0.73f,
                    dy1 = 1.75f,
                )
                // l -1.49 1.5
                lineToRelative(dx = -1.49f, dy = 1.5f)
                // l 1.42 1.4
                lineToRelative(dx = 1.42f, dy = 1.4f)
                // l 1.49 -1.48
                lineToRelative(dx = 1.49f, dy = -1.48f)
                // A 5 5 0 0 0 24 29.9
                arcTo(
                    horizontalEllipseRadius = 5.0f,
                    verticalEllipseRadius = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    x1 = 24.0f,
                    y1 = 29.9f,
                )
                // V 32
                verticalLineTo(y = 32.0f)
                // h 2
                horizontalLineToRelative(dx = 2.0f)
                // v -2.1
                verticalLineToRelative(dy = -2.1f)
                // a 5 5 0 0 0 1.75 -0.73
                arcToRelative(
                    a = 5.0f,
                    b = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    dx1 = 1.75f,
                    dy1 = -0.73f,
                )
                // l 1.5 1.49
                lineToRelative(dx = 1.5f, dy = 1.49f)
                // l 1.4 -1.42
                lineToRelative(dx = 1.4f, dy = -1.42f)
                // l -1.48 -1.49
                lineToRelative(dx = -1.48f, dy = -1.49f)
                // A 5 5 0 0 0 29.9 26z
                arcTo(
                    horizontalEllipseRadius = 5.0f,
                    verticalEllipseRadius = 5.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = false,
                    x1 = 29.9f,
                    y1 = 26.0f,
                )
                close()
                // m -7 2
                moveToRelative(dx = -7.0f, dy = 2.0f)
                // a 3 3 0 1 1 0 -6
                arcToRelative(
                    a = 3.0f,
                    b = 3.0f,
                    theta = 0.0f,
                    isMoreThanHalf = true,
                    isPositiveArc = true,
                    dx1 = 0.0f,
                    dy1 = -6.0f,
                )
                // a 3 3 0 0 1 0 6
                arcToRelative(
                    a = 3.0f,
                    b = 3.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = 0.0f,
                    dy1 = 6.0f,
                )
                // m -5 -11
                moveToRelative(dx = -5.0f, dy = -11.0f)
                // h -8
                horizontalLineToRelative(dx = -8.0f)
                // a 2 2 0 0 1 -2 -2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = -2.0f,
                    dy1 = -2.0f,
                )
                // V 4
                verticalLineTo(y = 4.0f)
                // a 2 2 0 0 1 2 -2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = 2.0f,
                    dy1 = -2.0f,
                )
                // h 8
                horizontalLineToRelative(dx = 8.0f)
                // a 2 2 0 0 1 2 2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = 2.0f,
                    dy1 = 2.0f,
                )
                // v 11
                verticalLineToRelative(dy = 11.0f)
                // a 2 2 0 0 1 -2 2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = -2.0f,
                    dy1 = 2.0f,
                )
                // m -8 -2
                moveToRelative(dx = -8.0f, dy = -2.0f)
                // h 8
                horizontalLineToRelative(dx = 8.0f)
                // V 4
                verticalLineTo(y = 4.0f)
                // h -8z
                horizontalLineToRelative(dx = -8.0f)
                close()
            }
            // M17 21 H8 a2 2 0 0 1 -2 -2 V7 h2 v12 h9z
            path(
                fill = SolidColor(Color(0xFF000000)),
            ) {
                // M 17 21
                moveTo(x = 17.0f, y = 21.0f)
                // H 8
                horizontalLineTo(x = 8.0f)
                // a 2 2 0 0 1 -2 -2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = -2.0f,
                    dy1 = -2.0f,
                )
                // V 7
                verticalLineTo(y = 7.0f)
                // h 2
                horizontalLineToRelative(dx = 2.0f)
                // v 12
                verticalLineToRelative(dy = 12.0f)
                // h 9z
                horizontalLineToRelative(dx = 9.0f)
                close()
            }
            // M13 25 H4 a2 2 0 0 1 -2 -2 V11 h2 v12 h9z
            path(
                fill = SolidColor(Color(0xFF000000)),
            ) {
                // M 13 25
                moveTo(x = 13.0f, y = 25.0f)
                // H 4
                horizontalLineTo(x = 4.0f)
                // a 2 2 0 0 1 -2 -2
                arcToRelative(
                    a = 2.0f,
                    b = 2.0f,
                    theta = 0.0f,
                    isMoreThanHalf = false,
                    isPositiveArc = true,
                    dx1 = -2.0f,
                    dy1 = -2.0f,
                )
                // V 11
                verticalLineTo(y = 11.0f)
                // h 2
                horizontalLineToRelative(dx = 2.0f)
                // v 12
                verticalLineToRelative(dy = 12.0f)
                // h 9z
                horizontalLineToRelative(dx = 9.0f)
                close()
            }
            // <rect width="32" height="32" fill="#000" style="fill:none" />
            path(
                fill = SolidColor(Color.Transparent),
            ) {
                // M 0 0
                moveTo(x = 0.0f, y = 0.0f)
                // h 32
                horizontalLineToRelative(dx = 32.0f)
                // v 32
                verticalLineToRelative(dy = 32.0f)
                // h -32z
                horizontalLineToRelative(dx = -32.0f)
                close()
            }
        }.build().also { _batchJob = it }
    }

@Suppress("ObjectPropertyName")
private var _batchJob: ImageVector? = null
